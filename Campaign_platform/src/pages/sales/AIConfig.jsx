    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold">AI Reference Material</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-sm font-medium transition"
          >
            <Plus size={15} /> New Business Unit
          </button>
        </div>
      </div>

      {/* ── How it works banner ── */}
      <div className="bg-indigo-950/40 border-b border-indigo-900/40 px-6 py-2.5 flex items-start gap-2 text-xs text-indigo-300">
        <Info size={14} className="mt-0.5 shrink-0" />
        <span>
          <strong>How Gemini + GPT-4 use these files:</strong>&nbsp;
            ...
            <nav className="flex-1 overflow-y-auto">
              {units.map(unit => (
                ...
                >
                  ...
                  {selected === unit.slug && <ChevronRight size={14} className="ml-auto shrink-0 text-indigo-400" />}
                </button>
              ))}
              {units.length === 0 && (
                <p className="px-4 py-3 text-xs text-gray-600">No configs yet. Create one →</p>
              )}
            </nav>
          )}
        </aside>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          {showNewForm && (
            <div className="bg-gray-900 border-b border-gray-800 px-6 py-4">
              <p className="text-sm font-medium mb-3">New Business Unit</p>
              <div className="flex gap-3 items-center">
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-400">Slug (used as filename, lowercase)</label>
                  <input
                    value={newSlug}
                    onChange={e => setNewSlug(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "_"))}
                    placeholder="e.g. bimwave or data_quality"
                    className="bg-gray-800 border border-gray-700 rounded-lg text-sm text-white px-3 py-2 w-64 focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <button
                  onClick={handleCreate}
                  disabled={!newSlug.trim()}
                  className="mt-5 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 rounded-lg text-sm font-medium transition"
                >
                  Create
                </button>
                <button
                  onClick={() => setShowNewForm(false)}
                  className="mt-5 px-3 py-2 text-gray-400 hover:text-white text-sm transition"
                >
                  Cancel
                </button>
              </div>
              <p className="mt-2 text-xs text-gray-500">
                A template will be pre-filled. Edit and save to activate.
              </p>
            </div>
          )}
          ...
          {selected && (
            <div className="bg-gray-900 border-b border-gray-800 px-6 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500 font-mono">{selected}.txt</span>
                {dirty && (
          ...
          {selected ? (
            <div className="flex-1 p-4 overflow-hidden">
              <textarea
                ...
                spellCheck={false}
                className="w-full h-full bg-gray-900 border border-gray-800 rounded-xl text-sm text-gray-100 font-mono p-5 resize-none focus:outline-none focus:border-indigo-700 leading-relaxed"
