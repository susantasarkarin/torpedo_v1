...
// Database fields for payments made
const DB_FIELDS_MADE = [
  { key: "payment_number", label: "Payment Number", required: false },
  { key: "vendor_name", label: "Vendor Name", required: true },
  { key: "bill_number", label: "Bill Number", required: false },
  { key: "amount", label: "Amount", required: true },
  { key: "currency_code", label: "Currency", required: false },
  { key: "payment_date", label: "Payment Date", required: true },
  { key: "payment_method", label: "Payment Method", required: false },
...
const COLUMN_ALIASES_RECEIVED = {
  payment_number: ["payment_number", "payment_no", "payment no", "payment#", "pmt_number", "pmt_no", "receipt_number", "receipt_no"],
  customer_name: ["customer_name", "customer", "client_name", "client", "company_name", "company", "received_from", "from"],
  invoice_number: ["invoice_number", "invoice_no", "invoice", "inv_number", "inv_no", "against_invoice"],
  amount: ["amount", "payment_amount", "received_amount", "total", "value"],
  currency_code: ["currency_code", "currency", "curr"],
  payment_date: ["payment_date", "date", "received_date", "receipt_date"],
  payment_method: ["payment_method", "method", "mode", "payment_mode", "type"],
  reference_number: ["reference_number", "reference", "ref", "ref_no", "transaction_id", "txn_id", "cheque_no", "check_no"],
  notes: ["notes", "remarks", "comments", "description", "memo"],
}

const COLUMN_ALIASES_MADE = {
  payment_number: ["payment_number", "payment_no", "payment no", "payment#", "pmt_number", "pmt_no"],
  vendor_name: ["vendor_name", "vendor", "supplier_name", "supplier", "company_name", "company", "paid_to", "to"],
  bill_number: ["bill_number", "bill_no", "bill", "against_bill", "vendor_invoice"],
  amount: ["amount", "payment_amount", "paid_amount", "total", "value"],
  currency_code: ["currency_code", "currency", "curr"],
...
function autoMatchColumns(csvColumns, columnAliases) {
  const mapping = {}
...
function PaymentsImport() {
  ...
  const initialType = searchParams.get("type") || "received"
  ...
  const DB_FIELDS = paymentType === "received" ? DB_FIELDS_RECEIVED : DB_FIELDS_MADE
  const COLUMN_ALIASES = paymentType === "received" ? COLUMN_ALIASES_RECEIVED : COLUMN_ALIASES_MADE
  const entityNameField = paymentType === "received" ? "customer_name" : "vendor_name"
  ...
  const handleFileSelect = (selectedFile) => {
    ...
    Papa.parse(selectedFile, {
      ...
      complete: (results) => {
        ...
        const autoMapping = autoMatchColumns(columns, COLUMN_ALIASES)
  ...
  const handleDownloadTemplate = () => {
    ...
    const sampleRow = paymentType === "received"
      ? "PMT-RCV-0001,Acme Corporation,INV-202512-0001,50000,INR,2025-12-21,bank_transfer,TXN123456,Payment for services"
      : "PMT-MADE-0001,Supplier Inc,BILL-202512-0001,25000,INR,2025-12-21,bank_transfer,TXN654321,Payment for goods"
    ...
    link.href = url
    link.download = `payments_${paymentType}_import_template.csv`
    link.click()
    URL.revokeObjectURL(url)
  }

  const handleImport = async () => {
    if (!columnMapping[entityNameField]) {
      setError(`${paymentType === "received" ? "Customer" : "Vendor"} Name field mapping is required`)
      return
    }
    
    if (!columnMapping.amount) {
      setError("Amount field mapping is required")
      return
    }
    
    if (!columnMapping.payment_date) {
      setError("Payment Date field mapping is required")
      return
    }
  ...
  const isFormValid = () => {
    return columnMapping[entityNameField] && columnMapping.amount && columnMapping.payment_date
  }

  return (
    <div style={styles.container}>
      {/* Back Button */}
      <button style={styles.backButton} onClick={() => navigate("/admin/finance/payments")}>
      ...
      <div style={styles.tabs}>
        <button
          style={{ ...styles.tab, ...(paymentType === "received" ? styles.tabActive : {}) }}
          onClick={() => handleTypeChange("received")}
        >
          Payments Received
        </button>
        <button
          style={{ ...styles.tab, ...(paymentType === "made" ? styles.tabActive : {}) }}
        ...
        <div style={styles.card}>
          <div style={{ textAlign: "center", marginBottom: "2rem" }}>
            ...
            <p style={{ color: "#6b7280" }}>
              Select a CSV file containing your {paymentType === "received" ? "payments received" : "payments made"} data
            </p>
          </div>
        ...
        <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <p style={{ fontSize: "0.875rem", color: "#6b7280", marginTop: "0.25rem" }}>
                    Match your CSV columns to payment fields. We've auto-matched what we could.
                  </p>
                </div>
          ...
          <div style={styles.actions}>
            ...
            <button
              ...
              style={{
                ...styles.btnPrimary,
                ...(importing || !isFormValid() ? styles.btnDisabled : {}),
              }}
            >
              {importing ? (
                <>
                ...
                <>
                  Import {csvData.length} Payments
        ...
        <div style={styles.card}>
          <div style={styles.successBox}>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: "1rem" }}>
              ...
              <button style={styles.btnPrimary} onClick={() => navigate("/admin/finance/payments")}>
                View Payments
              </button>
            </div>
          </div>
