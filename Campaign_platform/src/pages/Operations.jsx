...
function Operations() {
      ...
      if (trafficStatsResult.status === "fulfilled" && trafficStatsResult.value.ok) {
        ...
        if (statsData) {
          setTrafficStats({
            ...
            by_status_api_true: statsData.by_status_api_true || {},
            by_status_api_false: statsData.by_status_api_false || {},
            total_api_true: Number(statsData.total_api_true || 0),
            total_api_false: Number(statsData.total_api_false || 0),
  ...
  const renderDashboardV2 = () => {
    ...
    const getApiTrueCount = (statusKey) => Number(trafficStats?.by_status_api_true?.[statusKey] || 0);
    const getApiFalseCount = (statusKey) => Number(trafficStats?.by_status_api_false?.[statusKey] || 0);
    const totalTrafficRecords = Number(trafficStats?.total || 0);
    const totalApiTrue  = Number(trafficStats?.total_api_true  || 0);
    const totalApiFalse = Number(trafficStats?.total_api_false || 0);
    ...
    return (
      <div className="ops-reference-dashboard">
        ...
        {/* ── API Studies (CINT / CPX) ──────────────────────────────── */}
        <div className="ops-kpi-section-label">API Studies (CINT / CPX)</div>
        <div className="ops-kpi-grid">
          <div className="ops-kpi-card">
            <div className="ops-kpi-label">Total</div>
          ...
          <div className="ops-kpi-card">
            <div className="ops-kpi-label">Completes</div>
            <div className="ops-kpi-value-row">
          ...
          <div className="ops-kpi-card">
            <div className="ops-kpi-label">Partial</div>
            <div className="ops-kpi-value-row">
              <span className="ops-kpi-icon warning">P</span>
          ...
          <div className="ops-kpi-card">
            <div className="ops-kpi-label">CPX IR %</div>
            <div className="ops-kpi-value-row">
              <span className="ops-kpi-icon success">%</span>
              <span className="ops-kpi-value">{formatPercent(overallCpxIR)}</span>
            </div>
          </div>
          <div className="ops-kpi-card">
            <div className="ops-kpi-label">CINT IR %</div>
            <div className="ops-kpi-value-row">
              <span className="ops-kpi-icon info">%</span>
              <span className="ops-kpi-value">{formatPercent(overallCintIR)}</span>
            </div>
          </div>
        </div>

        {/* ── Non-API Studies (Projects) ────────────────────────────── */}
        <div className="ops-kpi-section-label" style={{ marginTop: "1.25rem" }}>Non-API Studies (Projects)</div>
        <div className="ops-kpi-grid">
          <div className="ops-kpi-card">
            <div className="ops-kpi-label">Total</div>
            <div className="ops-kpi-value-row">
