...
function GmailSetup() {
  ...
  const handleSyncWithProgress = async (mailboxId, autoClassifyAfter = false, skipRefresh = false) => {
    ...
    try {
      ...
      // Start the sync (runs in background now)
      ...
      if (res.ok) {
        ...
        if (data.status === "syncing" || data.message === "Sync started in background") {
          // Poll for progress
          const pollProgress = async () => {
            try {
              const statusRes = await fetch(`${API_BASE_URL}/gmail-ws/mailboxes/${mailboxId}/sync-status`, {
                headers: { Authorization: auth }
              })
              
              if (statusRes.ok) {
                const status = await statusRes.json()
                const synced = status.sync_progress || 0
                const total = status.sync_total || totalMessages || 1
                ...
                if (status.syncing) {
                  setSyncProgress(prev => ({
                    ...
                    [mailboxId]: { 
                      ...
                      synced: synced, 
                      percent: percent,
                      message: `Downloading emails: ${synced.toLocaleString()} / ${total.toLocaleString()} (${percent}%)` 
                    }
                  }))
                  ...
                  // Sync completed
                  const finalCount = status.email_count || synced
                  setSyncProgress(prev => ({
                    ...prev,
                    [mailboxId]: { 
                      status: "complete", 
                      ...
                      synced: finalCount, 
                      percent: 100,
                      message: `✅ Downloaded ${finalCount.toLocaleString()} emails` 
                    }
                  }))
                  
                  setMessage({ 
                    type: "success", 
                    text: `✅ Synced emails from ${emailAddress}` 
                  })
                  ...
                  if (autoClassifyAfter) {
                    setTimeout(() => runAutoClassification(), 1000)
                  }
          ...
          // Old-style immediate response (backwards compatibility)
                  ...
                  <div style={styles.mailboxInfo}>
                    ...
                    <div style={styles.mailboxMeta}>
                      <span style={styles.metaItem}>
                        📊 {(mailbox.gmail_total || mailbox.email_count || 0).toLocaleString()} emails
                      </span>
