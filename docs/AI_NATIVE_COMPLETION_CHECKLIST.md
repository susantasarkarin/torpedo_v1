...
|---|---|---|
| ~~**Local LLM inference on the prod VM**~~ | **RESOLVED (2026-09-09, EF-14)** — same tight VM (2 vCPU/3.8GB RAM, swap was 100% full at inspection time), same no-GPU constraint. Closed anyway: Qwen2.5-0.5B-Instruct (GGUF Q4_K_M, ~491MB) via llama.cpp, under a hard systemd cgroup memory ceiling (`MemoryMax=768M`, kills the process itself rather than risking the kernel OOM-killing v1 or Mongo), dedicated unprivileged system user, loopback-only + API-key-gated. Full detail: `docs/LOCAL_LLM_RUNBOOK.md`. | — (closed) |
...
| Actually renting a real GPU node | **NOT_STARTED — still blocked on `RUNPOD_API_KEY`**, confirmed absent from this environment. `GPU_BROKER_ENABLED` also unset (off by default, as designed). `docs/GPU_ACTIVATION_RUNBOOK.md` is the exact 18-step checklist to run once it's configured server-side. **No longer the only path to a working AI gateway** — see the next row: a real local model closes this for normal operation regardless of whether RunPod is ever activated |
| **Real local inference, no external LLM API required** (`app.ai.llm.LocalLlamaCppProvider`, `get_llm_provider()`) | **TESTED, deployed, live-verified against a real running model** (2026-09-09, EF-14) — Qwen2.5-0.5B-Instruct via llama.cpp, preferred over the RunPod path by `get_llm_provider()` (the new single factory replacing 7 separate construction sites across `emailai`/`finance`/`leadgen`/`panel` routers). `GET /ai/health` performs a real inference call, not a config check. `docs/LOCAL_LLM_RUNBOOK.md` has the full model/runtime/restart/security detail |
...
| `LLMProvider` Protocol + `GpuBrokerLLMProvider` + `LocalLlamaCppProvider` (`app.ai.llm`) | **TESTED** — fifth instance of this codebase's Protocol-boundary pattern, now with two real implementations (RunPod-leased and always-on local) selected by `get_llm_provider()`. 18 tests total across both providers + selection logic |
...
| Wiring `DecisionEngine.decide()` into real domain callers (email, AR follow-up, survey ranking, ...) | **TESTED** (2026-09-08) — stale entry, corrected: `EmailAIService`, `AIFinanceService` (AR/AP followup, payment matching), `PanelAllocationAIService`, `OperationsAIService`, `LeadGenAIService`, `LeadConversionAIService`, `OutreachAIService` all wired since Slices 12-19, each with its own dedicated test file, and each reachable from the Phase 14 scheduler's 9 event triggers, not just direct HTTP calls |
| Scheduled `sweep()` (idle/orphan reaping on a cadence) | **TESTED, deployed** (2026-09-08) — `POST /internal/gpu/sweep` (`app.ai.routers.gpu_sweep`), HMAC-signed with the same mechanism `/internal/scheduler/tick` uses (reusing `SCHEDULER_SIGNING_SECRET`, not a second secret), plus its own `torpedo-v2-gpu-sweep.timer` (every 5 minutes) — deliberately a separate timer, not folded into the per-org scheduler tick, because `GpuBroker.sweep()` is a global, deployment-wide operation with no `org_id` to scope it to, unlike everything else Phase 14 runs |
...
| Supplier/provider-failure-rate trigger | **TESTED** (2026-09-08) — `AllocationService.allocate()` now records a real `Activity` (`type="provider_timeout"`) on every `SurveyProviderUnavailable`, reusing the existing audit log rather than a new entity; `OperationsAIService.detect_triggers()` reads it back and raises a real `provider_failure_rate` operational trigger once `PROVIDER_FAILURE_THRESHOLD` (3) failures land within `PROVIDER_FAILURE_WINDOW` (24h) — both documented as adjustable operational parameters, not locked business rules, same as `HIGH_DROPOUT_THRESHOLD` |
| Client-response/deadline triggers | **TESTED** (2026-09-09, EF-13) — `Survey.client_deadline` + `deadline_passed`/`deadline_approaching` triggers; `pending_client_response_stale` from the real Activity trail (never `Survey.updated_at`). `REQUEST_CLIENT_STATUS` now resolves and records the real client contact via `Survey.opportunity_id -> Opportunity.person_id -> Person` — still never auto-sends, a deliberate separate decision. No separate `Study` entity needed (Slice 18 already settled that) |
| Change-request trigger | **NOT_STARTED, honestly** — no support-ticket or change-request data source exists anywhere in this codebase; fabricating one was rejected as exactly the no-fake-completion failure this rebuild exists to prevent |
...
| Deterministic detection → idempotent `Event` creation | **TESTED** — `app.scheduler.detectors.EventDetectionService`, nine real triggers as of 2026-09-08 (`app.scheduler.models.EVENT_TYPES` — this number has grown since first written; check that constant directly rather than trusting a count here), each reusing an existing detector/query, never reimplementing one |
