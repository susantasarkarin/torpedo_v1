...
def send(
    *,
    ...
    if not addr or "@" not in addr:
        return _blocked(f"invalid recipient address: {to_email!r}", "compliance")
    ...
    #
    # The generator now refuses to produce them (leads/bounce_recovery.py), but
    # the check belongs here too: this is the one door every sender goes
    # through, and addresses arrive from thirteen code paths and CSV imports,
    # not only from that generator.
    if _is_unsendable_domain(addr):
        return _blocked(
            f"unsendable domain: {addr.rsplit('@', 1)[-1]} — placeholder, "
            f"social or free-webmail address, never a corporate mailbox",
            "compliance")

    if not sending_enabled():
        return _blocked("SENDING_ENABLED=false (kill switch active)", "disabled")

    problem = _compliance_problem(transactional)
    if problem:
        return _blocked(problem, "compliance")
