...
def _is_unsendable_domain(addr: str, allow_free_webmail: bool = False) -> bool:
    ...
    return is_unsendable_domain(addr, allow_free_webmail=allow_free_webmail)
...
def gate(addr: str, *, identity: str, channel: str,
         ...
         check_budget: bool = True, allow_free_webmail: bool = False,
    ...
    # allow_free_webmail=True for consumer audiences (the panel): a gmail
    # address is a real person there, and blocking it would be the bug.
    if _is_unsendable_domain(addr, allow_free_webmail=allow_free_webmail):
    ...
    #
    # check_budget=False is for a sender that already enforces its own volume
    # cap and whose real volume exceeds this module's defaults. The panel
    # invitation job is the case: it sends ~11,300/day under its own daily cap,
    # while SEND_BUDGET_DEFAULT_DAILY is 1,500 — applying that here would cut it
    # by 88% without anyone asking for it. Suppression and the kill switch still
    # apply, which is the part it was missing. Give the identity an explicit
    # SEND_BUDGET_<IDENTITY>_DAILY and turn this back on.
    if check_budget:
