"""
leads/routers/discover.py
==========================
Route group: AI company discovery (Perplexity + Google CSE) and two-phase
company/contact discovery pipeline.
Registered via register_discover_routes(router) in leads/router.py.
"""
...
def register_discover_routes(router: APIRouter):
    ...
    async def get_target_industries():
        """GET /leads/discover/industries — Get list of target industries."""
        from ..ingestion import TARGET_INDUSTRIES
        return {"industries": TARGET_INDUSTRIES, "total": len(TARGET_INDUSTRIES)}

    @router.get("/discover/regions")
    async def get_target_regions():
        """GET /leads/discover/regions — Get list of target regions."""
        from ..ingestion import TARGET_REGIONS
        return {"regions": TARGET_REGIONS, "total": len(TARGET_REGIONS)}
    ...
    async def discover_company_contacts_endpoint(request: ContactDiscoveryRequest):
        ...
        For research companies: Finds Operations/Field managers
        """
        try:
            from ..ingestion import find_decision_makers_in_company

            all_leads = []
            stats = {"companies_searched": 0, "total_contacts": 0, "errors": []}
    ...
    async def run_full_discovery_endpoint(request: FullDiscoveryRequest):
        ...
        Run complete two-phase discovery (find companies then decision makers).
        """
        try:
    ...
    async def import_discovered_leads_endpoint(request: dict):
        """POST /leads/discover/import-leads — Import leads from two-phase discovery."""
        try:
            from ..models import LeadRaw
            from ..service import import_leads
