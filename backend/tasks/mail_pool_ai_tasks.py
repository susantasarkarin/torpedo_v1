...
from sales.mail_ai_switch import disabled_result, mail_ai_enabled
...
def process_mail_pool_batch(self, limit: int = None):
    ...
    if not mail_ai_enabled():
        logger.info("[mail-ai] mail-pool batch skipped: mail-AI is switched off")
        return disabled_result("mail-pool batch")
    try:
...
def process_mail_pool_sender_batch(self, limit: int = 50):
    ...
    """
    if not mail_ai_enabled():
        logger.info("[mail-ai] mail-pool sender batch skipped: mail-AI is switched off")
        return disabled_result("mail-pool sender batch")
    try:
...
def rebuild_rfqs_all_senders_batch(self, limit: int = 50):
    ...
    """
    if not mail_ai_enabled():
        logger.info("[mail-ai] RFQ rebuild batch skipped: mail-AI is switched off")
        return disabled_result("RFQ rebuild batch")
    try:
...
def audit_prefiltered_mail(self, sample: int = None):
    ...
    if not mail_ai_enabled():
        logger.info("[mail-ai] prefilter audit skipped: mail-AI is switched off")
        return disabled_result("prefilter audit")
    try:
