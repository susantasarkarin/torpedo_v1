    ...
    try:
        ...
        for idx, row in enumerate(reader, start=2):
            try:
                ...
                existing = customers_collection.find_one({"name": row.get("name", "").strip()})
                if existing:
                    errors.append(f"Row {idx}: Customer '{row.get('name')}' already exists")
                    continue
                ...
                customer_data = {
                    ...
                    "email": row.get("email", ""),
                    "phone": row.get("phone", ""),
                    "gst_treatment": row.get("gst_treatment", "unregistered"),
                    "gstin": row.get("gstin", ""),
                    "pan": row.get("pan", ""),
                    "billing_address": {
                        "line1": row.get("billing_address_line1", ""),
                        "city": row.get("billing_address_city", ""),
                        "state": row.get("billing_address_state", ""),
                        "pincode": row.get("billing_address_pincode", ""),
                        "country": "India"
                    },
...
async def import_vendors_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        for idx, row in enumerate(reader, start=2):
            try:
                existing = vendors_collection.find_one({"name": row.get("name", "").strip()})
                if existing:
                    errors.append(f"Row {idx}: Vendor '{row.get('name')}' already exists")
                    continue
    ...
    try:
        ...
        for idx, row in enumerate(reader, start=2):  # Start at 2 for Excel row reference (header is row 1)
            try:
                ...
                existing = invoices_collection.find_one({"invoice_number": row.get("invoice_number", "")})
                if existing:
                    errors.append(f"Row {idx}: Invoice '{row.get('invoice_number')}' already exists")
                    continue
                ...
                # Create invoice data
                invoice_data = {
                    "invoice_number": row.get("invoice_number") or generate_invoice_number(),
                    ...
                    "discount_type": row.get("discount_type", "flat"),
    ...
    try:
        ...
        for idx, row in enumerate(reader, start=2):
            try:
                ...
                existing = bills_collection.find_one({"bill_number": row.get("bill_number", "")})
                if existing:
                    errors.append(f"Row {idx}: Bill '{row.get('bill_number')}' already exists")
                    continue
                ...
                bill_data = {
                    "bill_number": row.get("bill_number") or generate_bill_number(),
                    "vendor_id": vendor_id,
