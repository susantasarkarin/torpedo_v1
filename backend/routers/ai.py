...
async def submit_decision(payload: Dict[str, Any] = Body(...), _user: str = Depends(require_auth)):
...
async def get_queue_item(queue_id: str, _user: str = Depends(require_auth)):
    try:
...
async def approve(queue_id: str, user: str = Depends(require_auth)):
...
async def reject(queue_id: str, payload: Dict[str, Any] = Body(default={}), user: str = Depends(require_auth)):
