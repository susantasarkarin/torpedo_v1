...
@router.get("/api/traffic/surveys-stats")
async def get_all_surveys_traffic_stats(request: Request):
    ...
    Returns a dictionary mapping survey_id -> {"clicks": int, "completes": int}
    Requires authentication
    """
    try:
        # Verify session
        ...
        stats = traffic_service.get_all_surveys_traffic_stats()
        return {"surveys_stats": stats}
