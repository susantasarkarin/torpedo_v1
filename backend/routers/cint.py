    ...
    try:
        ...
        async def _process_webhook_background():
            ...
            try:
                ...
                # during CINT fallback in traffic.py, not eagerly here.
...
# Define IR ranges (rows) - Fixed boundary gaps and labels
IR_RANGES = [
...
# Define LOI ranges (columns) - Fixed boundary overlaps
LOI_RANGES = [
...
def normalize_country(value: Any) -> Optional[str]:
    ...
    if isinstance(value, (int, float)):
        return CINT_COUNTRY_LANGUAGE_MAP.get(int(value))
    ...
    if isinstance(value, str):
        ...
        if not raw:
            return None
        if raw.isdigit():
            return CINT_COUNTRY_LANGUAGE_MAP.get(int(raw))
        if "_" in raw:
            return raw.split("_")[-1].upper()
        if "-" in raw:
...
async def get_rate_card(
    ...
    try:
        ...
        # Extract country from country_language (e.g., "eng_us" -> "US" or numeric ID)
        def extract_country(survey):
            cl = survey.get("country_language") or survey.get("country")
            return normalize_country(cl)
        ...
        for survey in all_surveys:
            ...
            # Find IR range index - use < for max boundary except last range
            ir_idx = None
            for i, r in enumerate(IR_RANGES):
                is_last = (i == len(IR_RANGES) - 1)
                if is_last:
                    if r["min"] <= ir <= r["max"]:
                        ir_idx = i
                        break
            ...
            # Find LOI range index - use < for max boundary except last range
            loi_idx = None
            for j, r in enumerate(LOI_RANGES):
                is_last = (j == len(LOI_RANGES) - 1)
                if is_last:
                    if r["min"] <= loi <= r["max"]:
                        loi_idx = j
                        break
