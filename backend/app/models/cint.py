...
class SupplierLinkBase(BaseModel):
    ...
    supplier_link_type_code: SupplierLinkType = Field(..., description="Type of engagement")
    tracking_type_code: TrackingType = Field(default=TrackingType.NONE, description="Communication method")
    
    # Redirect URLs (max 2999 characters each)
    default_link: Optional[str] = None
    success_link: Optional[str] = None
    failure_link: Optional[str] = None
    over_quota_link: Optional[str] = None
    quality_termination_link: Optional[str] = None
...
class SupplierLink(SupplierLinkBase):
    ...
    # Payout information
    rpi: Optional[Dict[str, Any]] = None  # {"value": 1.35, "currency_code": "USD"}
