...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: ./deploy.sh [--frontend-only|--backend-only] [--no-pull] [--no-confirm] [--force]"
            exit 1
            ;;
    esac
done
...
deploy_frontend() {
    ...
    if [ "$USE_APACHE" = true ] || [ "$USE_NGINX" = true ]; then
        ...
        if [ -d "$FRONTEND_WEB_ROOT" ] && [ "$(ls -A $FRONTEND_WEB_ROOT 2>/dev/null)" ]; then
            BACKUP_DIR="${FRONTEND_WEB_ROOT}.backup.$(date +%Y%m%d_%H%M%S)"
...
show_summary() {
    ...
    if [ "$DEPLOY_FRONTEND" = true ]; then
        ...
        if [ "$USE_APACHE" = true ] || [ "$USE_NGINX" = true ]; then
            echo -e "    URL:     http://localhost (or your configured domain)"
        else
            echo -e "    Build:   $FRONTEND_DIST"
        fi
    fi
    ...
    if [ "$DEPLOY_BACKEND" = true ]; then
        ...
        echo -e "    URL:     http://localhost:9944"
    fi
    
    echo ""
    echo "Useful commands:"
    if [ "$DEPLOY_FRONTEND" = true ]; then
        echo "  View frontend build:  ls -la $FRONTEND_DIST"
        if [ "$USE_APACHE" = true ]; then
    ...
    if [ "$DEPLOY_BACKEND" = true ]; then
        if [ "$USE_SYSTEMD" = true ]; then
            echo "  View backend logs:    sudo journalctl -u $BACKEND_SERVICE_NAME -f"
        else
