...
async def get_mail_pool_stats(
    request: Request,
    account: Optional[str] = Query(None, description="Filter stats by account email")
):
    """
    ...
    Optionally filter by account email.
    """
    try:
        ...
        base_filter = {}
        if account:
            # Find the account's mailbox_id
            account_doc = mail_pool_workspace_mailboxes.find_one({"email": account.lower()})
            if not account_doc:
                account_doc = mail_pool_mailboxes.find_one({"email": account.lower()})
            if account_doc:
                base_filter["mailbox_id"] = str(account_doc["_id"])
            else:
                # Fallback: try matching by from_email
                base_filter["$or"] = [
                    {"from_email": {"$regex": account, "$options": "i"}},
                    {"to_email": {"$regex": account, "$options": "i"}},
                    {"mailbox_email": account.lower()}
                ]
        
        # Total emails from email_automation.emails
        total_emails = mail_pool_emails.count_documents(base_filter)
        
        # Count by direction for inbox/sent breakdown (new schema uses 'direction' field)
        inbox_filter = {**base_filter, "direction": "inbound"}
        sent_filter = {**base_filter, "direction": "outbound"}
        inbox_count = mail_pool_emails.count_documents(inbox_filter)
        sent_count = mail_pool_emails.count_documents(sent_filter)
        
        # Count drafts
        drafts_filter = {**base_filter, "labels": {"$regex": "DRAFT", "$options": "i"}}
        drafts_count = mail_pool_emails.count_documents(drafts_filter)
        
        # Get category breakdown (acts as segments) - new schema uses 'category' field
        category_match = {"$match": base_filter} if base_filter else {"$match": {}}
        category_pipeline = [
            category_match,
        ...
        ai_base_match = {**base_filter} if base_filter else {}
        ai_base_match["$or"] = [
